...
public class HomeController implements Initializable {
    ...
    public void setUpListeners() {
        ...
        paneChangeRole.expandedProperty().addListener((obs, wasExpanded, isNowExpanded) -> {
            if (isNowExpanded) {
                fillTable(changeRoleUsersTable, PeercheckRESTController.getAllUsers());
            }
        });
        ...
        paneAssignReviewer.expandedProperty().addListener((obs, wasExpanded, isNowExpanded) -> {
            if (isNowExpanded) {
                fillTable(assignReviewerArticlesTable, PeercheckRESTController.getAllArticles());
                fillTable(assignReviewerUsersTable, PeercheckRESTController.findUsersByRole("REVISOR"));
            }
        });
        ...
        paneSendReview.expandedProperty().addListener((obs, wasExpanded, isNowExpanded) -> {
            if (isNowExpanded) {
                fillTable(sendReviewReviewsTable, PeercheckRESTController.getReviewsByReviewer(UserSession.user));
            }
        });
        ...
        paneViewGrade.expandedProperty().addListener((obs, wasExpanded, isNowExpanded) -> {
            if (isNowExpanded) {
                fillTable(viewGradeArticlesTable, PeercheckRESTController.getArticlesByAuthor(UserSession.user));
            }
        });
        ...
        viewGradeArticlesTable.setOnMouseClicked((MouseEvent event) -> {
            if (event.getButton().equals(MouseButton.PRIMARY)) {
                if (viewGradeArticlesTable.getSelectionModel().getSelectedItem() != null) {
                    ...
                    fillTable(viewGradeReviewsTable, PeercheckRESTController.getReviewsByArticle(viewGradeArticlesTable.getSelectionModel().getSelectedItem()));
                    //Actualizar promedio de calificación
                    viewGradeLabel.setText(String.format("%1.2f", PeercheckRESTController.calculateFinalGradeToArticle(viewGradeArticlesTable.getSelectionModel().getSelectedItem())));
                }
            }
        });
        ...
        paneCreateArticle.expandedProperty().addListener((obs, wasExpanded, isNowExpanded) -> {
            if (isNowExpanded) {
                fillTable(createArticleEventsTable, PeercheckRESTController.getAllEvents());
            }
        });
        ...
        paneSearchEvent.expandedProperty().addListener((obs, wasExpanded, isNowExpanded) -> {
            if (isNowExpanded) {
                fillTable(searchEventEventsTable, PeercheckRESTController.getAllEvents());
            }
        });
    }
    ...
    void findArticle(ActionEvent event) {
        ...
        List<Articles> articles = PeercheckRESTController.findArticleBy(criteria, query);
    ...
    void downloadFile() {
        if (searchArticlesTable.getSelectionModel().getSelectedItem() != null) {
            ...
            try {
                TrannyFile trannyFile = PeercheckRESTController.getArticleFile(searchArticlesTable.getSelectionModel().getSelectedItem());
                if (trannyFile != null) {
    ...
    void changeRole() {
        if (UserSession.user.getRole().equals("EDITOR")) {
            ...
            PeercheckRESTController.changeRol(selectedUser, newRole);
            fillTable(changeRoleUsersTable, PeercheckRESTController.getAllUsers());
        } else {
    ...
    void createArticle() {
        ...
        PeercheckRESTController.addArticle(article, fileToUpload);
    ...
    void assignReviewer() {
		...
		PeercheckRESTController.addReview(newReview);
    ...
    void sendReview() {
		...
		if (review != null) {
			...
			PeercheckRESTController.updateReview(review);
    ...
    void createEvent() {
		...
		event.setDeadline(date);
		event.setEditorId(UserSession.user);
		event.setSubmissionstart(new Date());

		PeercheckRESTController.addEvent(event);
